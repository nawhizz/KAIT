{*---------------------------------------------------------------------------
프로그램ID    : losta530L (입고내역LIST(습득자내역))
프로그램 종류 : Online
작성자	      : 정 홍 렬
작성일	      : 2011. 08. 24
완료일	      : ####. ##. ##
...
const
  TITLE   = '입고내역LIST(습득자내역)';
  PGM_ID  = 'LOSTA530L';

type
  Tfrm_LOSTA530L = class(TForm)
    Bevel2: TBevel;
...
var
  frm_LOSTA530L: Tfrm_LOSTA530L;

implementation
uses  u_landprt;
{$R *.DFM}

procedure Tfrm_LOSTA530L.disableComponents;
begin
...
procedure Tfrm_LOSTA530L.enableComponents;
begin
  changeBtn(Self);
...
procedure Tfrm_LOSTA530L.PrtFormShow;
begin
	// 출력 다이로그 박스
...
procedure Tfrm_LOSTA530L.setEdtKeyPress;
var i : Integer;
begin
...
procedure Tfrm_LOSTA530L.Edt_onKeyPress ( Sender : TObject; var key : Char);
begin
 if (key = #13) then
...
{----------------------------------------------------------------------------}
procedure Tfrm_LOSTA530L.FormCreate(Sender: TObject);
begin
  {----------------------- 공통 어플리케이션 설정 ---------------------------}
   setEdtKeyPress;
...
procedure Tfrm_LOSTA530L.btn_CloseClick(Sender: TObject);
begin
     close;
end;

procedure Tfrm_LOSTA530L.dte_fromExit(Sender: TObject);
begin
...
procedure Tfrm_LOSTA530L.dte_toExit(Sender: TObject);
begin
	sts_Message.Panels[1].Text := '';
...
procedure Tfrm_LOSTA530L.btn_PrintClick(Sender: TObject);
var
...
begin
  ...
  TMAX.Connect;
	...
	if (TMAX.SendString('INF003','LOSTA530L') < 0) then  goto LIQUIDATION;
  ...
  serviceSuccess := TMAX.Call('LOSTA530L');
...
procedure Tfrm_LOSTA530L.btn_queryClick(Sender: TObject);
var
	cmdStr:String;
    filePath:String;
    f:TextFile;
begin
    ...
    filePath:='..\Temp\LOSTA530L_QRY.txt';
...
procedure Tfrm_LOSTA530L.btn_ResetClick(Sender: TObject);
begin
  dte_from.Date := date;
  dte_to.Date := date;
  changeBtn(Self);
