...
function Tfrm_LOSTD120P.keyCheck:boolean;
begin
	result:= True;
...
procedure Tfrm_LOSTD120P.AttachOnKeyPressEvent;
begin
...
(*                                                                            *)
(******************************************************************************)
procedure Tfrm_LOSTD120P.OnKeyPress(Sender: TObject; var Key: Char);
var
  edit:TEdit;
  dedit:TDateEdit;
  medit:TMaskEdit;
...
(*                                                                            *)
(******************************************************************************)
procedure Tfrm_LOSTD120P.OnSearchClick(Sender:TObject);
begin
    //성명
    if Sender = btn1 then
      begin
...
(******************************************************************************)
procedure Tfrm_LOSTD120P.OnExit(Sender:TObject);
var
  edit  :TEdit;
  dedit :TDateEdit;
  medit :TMaskEdit;
...
(*                                                                            *)
(******************************************************************************)
procedure Tfrm_LOSTD120P.DetachOnEnterEvent;
begin
...
(*                                                                            *)
(******************************************************************************)
procedure Tfrm_LOSTD120P.AttachOnExitEvent;
begin
...
(*                                                                            *)
(******************************************************************************)
procedure Tfrm_LOSTD120P.AttachOnEnterEvent;
begin
...
(******************************************************************************)
procedure Tfrm_LOSTD120P.OnEnter(Sender:TObject);
var
  edit  :TEdit;
  dedit :TDateEdit;
  medit :TMaskEdit;
...
(*                                                                            *)
(******************************************************************************)
procedure Tfrm_LOSTD120P.SetItemNo(number:String);
begin
    itemNo  := number;
    value1  :='';
    value2  :='';
...
procedure Tfrm_LOSTD120P.Link_rtn (var Msg : TMessage);
var
  (**************************************************)
  (*  localcloud.pas                                *)
  (*  type :TPSharedMem                             *)
...
// 모델콤보 클릭시 이벤트
procedure Tfrm_LOSTD120P.md_cb1ButtonClick(Sender: TObject);
begin
...
// 모델 콤보 KeyUp 이벤트
procedure Tfrm_LOSTD120P.md_cb1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  i : integer;
begin
...
// 모델 그리드 클릭시
procedure Tfrm_LOSTD120P.md_grid1Click(Sender: TObject);
begin
   md_cb1.text := md_grid1.Cells[0,md_grid1.row];
   md_cb1.SetFocus;
   md_grid1.Visible := false;
...
procedure Tfrm_LOSTD120P.FormCreate(Sender: TObject);
begin
...
(*                                                                            *)
(******************************************************************************)
procedure Tfrm_LOSTD120P.InitComponents;
var
  i : Integer;
...
procedure Tfrm_LOSTD120P.btn_AddClick(Sender: TObject);
...
begin
  ...
  TMAX.Connect;
	...
	if (TMAX.SendString('INF003','LOSTD120P')       < 0) then  goto LIQUIDATION;
    ...
    //서비스 호출
	if not TMAX.Call('LOSTD120P') then
  begin
...
//'수정' 버튼 클릭
procedure Tfrm_LOSTD120P.btn_UpdateClick(Sender: TObject);
begin
  btn_AddClick(Sender);
end;

procedure Tfrm_LOSTD120P.btn_DeleteClick(Sender: TObject);
	Label LIQUIDATION;
begin
...
procedure Tfrm_LOSTD120P.btn_InquiryClick(Sender: TObject);
...
begin
  ...
  (* 가입자주민사업자번호    *)   lbl_Lt_No.Caption :=           Trim(TMAX.RecvString('STR113',0));
  (* 가입자성명(업체명)      *)   lbl_Lt_Nm.Caption :=           Trim(TMAX.RecvString('STR114',0));
  ...
  (* 가입자전화번호          *)   lbl_Tl_No.Caption := InsHyphen(Trim(TMAX.RecvString('STR118',0)));
...
procedure Tfrm_LOSTD120P.btn_CloseClick(Sender: TObject);
begin
    //호출한 APP가 살이 있으면 메세지....
...
//On-Exit (입고일자)
procedure Tfrm_LOSTD120P.dte_Ip_DtExit(Sender: TObject);
begin
{
     try
...
function Tfrm_LOSTD120P.ExecExternProg(progID:String):Boolean;
var
	commandStr:String;
	ret:Integer;
begin
...
procedure Tfrm_LOSTD120P.FormShow(Sender: TObject);
begin
  //콤포넌트 초기화
  InitComponents;
end;

procedure Tfrm_LOSTD120P.btn_resetClick(Sender: TObject);
begin
  self.InitComponents;
end;
