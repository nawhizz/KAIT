  ...
  if count1 < 1 then begin
    ...
    with grd_display do begin
    	for i:=0 to count1-1 do begin
          ...
          Cells[1,RowPos]  := TMAX.RecvString('STR102',i);     // 경찰서명
          Cells[2,RowPos]  := TMAX.RecvString('STR103',i);     // 전화번호
          Cells[3,RowPos]  := TMAX.RecvString('STR104',i);     // 관리번호
          Cells[4,RowPos]  := TMAX.RecvString('STR105',i);     // 획득순번
          Cells[5,RowPos]  := TMAX.RecvString('STR106',i);     // 사업자명
          Cells[6,RowPos]  := TMAX.RecvString('STR107',i);     // 모델명
          Cells[7,RowPos]  := TMAX.RecvString('STR108',i);     // 일련번호
          seed_mtno        := TMAX.RecvString('STR109',i);     // 이동전화번호
          Cells[8,RowPos]  := ECPlazaSeed.Decrypt(seed_mtno, common_seedkey);
          seed_gano        := TMAX.RecvString('STR110',i);     // 가입자번호
          Cells[9,RowPos]  := ECPlazaSeed.Decrypt(seed_gano, common_seedkey);
          seed_ganm        := TMAX.RecvString('STR111',i);     // 가입자성명
          Cells[10,RowPos] := ECPlazaSeed.Decrypt(seed_ganm, common_seedkey);
          seed_gatl        := TMAX.RecvString('STR112',i);     // 가입자전화번호
          Cells[11,RowPos] := ECPlazaSeed.Decrypt(seed_gatl, common_seedkey);
          Cells[12,RowPos] := TMAX.RecvString('STR113',i);     // 가입자주소
          seed_nano        := TMAX.RecvString('STR114',i);     // 납부자번호
          Cells[13,RowPos] := ECPlazaSeed.Decrypt(seed_nano, common_seedkey);
          seed_nanm        := TMAX.RecvString('STR115',i);     // 납부자성명
          Cells[14,RowPos] := ECPlazaSeed.Decrypt(seed_nanm, common_seedkey);
          seed_natl        := TMAX.RecvString('STR116',i);     // 납부자전화번호
          Cells[15,RowPos] := ECPlazaSeed.Decrypt(seed_natl, common_seedkey);
          Cells[16,RowPos] := TMAX.RecvString('STR117',i);     // 납부자 주소
          Cells[17,RowPos] := TMAX.RecvString('STR118',i);     // 보험금 상태
