{*---------------------------------------------------------------------------
프로그램ID    : LOSTD510L (전달폰 출고입력 단말기 출력)
프로그램 종류 : Online
작성자	      : 정 홍 렬
작성일	      : 2011. 09. 01
완료일	      : ####. ##. ##
...
var
  frm_LOSTD510L: Tfrm_LOSTD510L;

implementation
uses u_landprt;
{$R *.DFM}

procedure Tfrm_LOSTD510L.disableComponents;
begin
  disableBtn(Self);
...
procedure Tfrm_LOSTD510L.enableComponents;
begin
  changeBtn(Self);
...
procedure Tfrm_LOSTD510L.setEdtKeyPress;
var i : Integer;
begin
...
procedure Tfrm_LOSTD510L.Edt_onKeyPress ( Sender : TObject; var key : Char);
begin
 if (key = #13) then
   SelectNext( ActiveControl as TEdit , true, True);
end;
procedure Tfrm_LOSTD510L.PrtFormShow;
begin
	// 출력 다이로그 박스
	frm_LANDPRT := Tfrm_LANDPRT.Create(self);
	frm_LANDPRT.FormSet('LOSTD510L.txt', lbl_Program_Name.Caption);
	try
...
{----------------------------------------------------------------------------}
procedure Tfrm_LOSTD510L.FormCreate(Sender: TObject);
begin
  {----------------------- 공통 어플리케이션 설정 ---------------------------}
   setEdtKeyPress;
...
procedure Tfrm_LOSTD510L.btn_CloseClick(Sender: TObject);
begin
     close;
end;

procedure Tfrm_LOSTD510L.dte_fromExit(Sender: TObject);
begin
...
procedure Tfrm_LOSTD510L.dte_toExit(Sender: TObject);
begin
	sts_Message.Panels[1].Text := '';
...
procedure Tfrm_LOSTD510L.btn_PrintClick(Sender: TObject);
var
  count1,i : Integer;
...
begin
  ...
  TMAX.Connect;
	...
	if (TMAX.SendString('INF003','LOSTD510L') < 0) then  goto LIQUIDATION;
  ...
  serviceSuccess := TMAX.Call('LOSTD510L');
...
procedure Tfrm_LOSTD510L.btn_ResetClick(Sender: TObject);
begin
	dte_from.Date := date;
	dte_to.Date := date;
...
procedure Tfrm_LOSTD510L.FormShow(Sender: TObject);
begin
  btn_ResetClick(Sender);
end;

procedure Tfrm_LOSTD510L.btn_queryClick(Sender: TObject);
var
	cmdStr:String;
    filePath:String;
    f:TextFile;
begin
    ...
    filePath:='..\Temp\LOSTD510L_QRY.txt';
