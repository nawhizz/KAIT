...
uses
  ...
  Dialogs, StdCtrls, jpeg, ExtCtrls, WinSkinData, Menus, Buttons, IniFiles,
  u_mainmenu;

const
	SKINDIR = '..\Skin';   //실행 프로그램은 '..\bin'에 있다고 가정한다.
    INIDIR = '..\Ini';
type
...
begin
	...
	// 상대경로 지정 방식
	GetSearchedFileList(SKINDIR +'\Skin_picture', slFileList, '*' + locTar + '.jpg', false);

	if (slFileList.Count <> 0) then begin
		locTar := SKINDIR +'\Skin_picture\' + slFileList.Strings[0];
...
begin
	...
	GetSearchedFileList(SKINDIR, slFileList, '*.skn', false);
...
begin
    if(ListBox1.ItemIndex <> -1) Then  begin
		frm_MAINMENU.SkinData1.SkinFile := SKINDIR+'\' + ListBox1.Items.Strings[ListBox1.ItemIndex];

		if not frm_MAINMENU.SkinData1.Active then
			frm_MAINMENU.SkinData1.Active:=true;

        // 정식 INI 설정 방법
        iniLost := TiniFile.Create(INIDIR +'\LostProj.ini');
