...
const
  TITLE   = '보험금 지급이후 입고 단말기 조회';
  PGM_ID  = 'LOSTE240Q';

type
  Tfrm_LOSTE240Q = class(TForm)
    Bevel1     : TBevel;
    Bevel2     : TBevel;
    dte_to     : TDateEdit;
    dte_from   : TDateEdit;
...
procedure Tfrm_LOSTE240Q.disableComponents;
begin
...
procedure Tfrm_LOSTE240Q.enableComponents;
begin
...
procedure Tfrm_LOSTE240Q.initStrGrid;
begin
	with grd_display do
  begin
    RowCount      :=  2;
    ...
    ColWidths[10] := 115;  // 승인일자
    ColWidths[11] := 150;  // 보험금지급액
    ColWidths[12] := 110;  // 입출고구분
    ColWidths[13] := 110;  // 입고일자
    ColWidths[14] := 110;  // 출고일자
    ColWidths[15] := 120;  // 입고상태
    ColWidths[16] := 110;  // 접수우체국
    ColWidths[17] := 110;  // 출고처리구분
    ColWidths[18] := 110;  // 단말기처리상태
    ColWidths[19] := 100;   // 경찰서습득단말기여부
...
procedure Tfrm_LOSTE240Q.setEdtKeyPress;
var i : Integer;
begin
...
procedure Tfrm_LOSTE240Q.Edt_onKeyPress ( Sender : TObject; var key : Char);
begin
 if (key = #13) then
...
{----------------------------------------------------------------------------}
procedure Tfrm_LOSTE240Q.btn_CloseClick(Sender: TObject);
begin
     close;
end;

procedure Tfrm_LOSTE240Q.FormCreate(Sender: TObject);
begin
  begin
  {----------------------- 공통 어플리케이션 설정 ---------------------------}
     setEdtKeyPress;
...
procedure Tfrm_LOSTE240Q.btn_InquiryClick(Sender: TObject);
var
    // 암호화 관련 변수
...
INQUIRY:
  ...
  begin
    ...
    {* 보험사        *}  Cells[1  ,RowPos]   := TMAX.RecvString('STR101',i);
    {* 이통사        *}  Cells[2  ,RowPos]   := TMAX.RecvString('STR102',i);
    {* 모델명        *}  Cells[3  ,RowPos]   := TMAX.RecvString('STR103',i);
    {* 일련번호      *}  Cells[4  ,RowPos]   := TMAX.RecvString('STR104',i);
    {* 이동전화번호  *}  seed_mtno           := TMAX.RecvString('STR105',i);
                         Cells[5  ,RowPos]   := ECPlazaSeed.Decrypt(seed_mtno, common_seedkey);
    {* 가입자성명    *}  seed_ganm           := TMAX.RecvString('STR106',i);
    ...
    {* 보험금신청일자*}  Cells[9  ,RowPos]   := InsHyphen(TMAX.RecvString('STR109',i));
    {* 승인일자      *}  Cells[10 ,RowPos]   := InsHyphen(TMAX.RecvString('STR111',i));
    {* 보험금지급액  *}  Cells[11 ,RowPos]   := convertWithCommer(IntToStr(Round(TMAX.RecvDouble('DBL112',i))));
    {* 입출고구분    *}  Cells[12 ,RowPos]   := TMAX.RecvString('STR113',i);
    {* 입고일자      *}  Cells[13 ,RowPos]   := InsHyphen(TMAX.RecvString('STR114',i));
    {* 출고일자      *}  Cells[14 ,RowPos]   := InsHyphen(TMAX.RecvString('STR115',i));
    {* 입고상태      *}  Cells[15 ,RowPos]   := TMAX.RecvString('STR116',i);
    {* 접수우체국    *}  Cells[16 ,RowPos]   := TMAX.RecvString('STR117',i);
    {* 출고처리구분  *}  Cells[17 ,RowPos]   := TMAX.RecvString('STR118',i);
    {* 단말기처리상태*}  Cells[18 ,RowPos]   := TMAX.RecvString('STR120',i);
    {* 경찰서접수    *}  Cells[19 ,RowPos]   := TMAX.RecvString('STR121',i);
...
procedure Tfrm_LOSTE240Q.FormShow(Sender: TObject);
begin
  InitComponent;
end;

procedure Tfrm_LOSTE240Q.btn_queryClick(Sender: TObject);
var
	cmdStr    :String;
  filePath  :String;
  f:TextFile;
...
procedure Tfrm_LOSTE240Q.btn_excelClick(Sender: TObject);
begin
  Proc_gridtoexcel(TITLE + '(' + PGM_ID + ')',grd_display.RowCount, grd_display.ColCount, grd_display, PGM_ID);
end;

procedure Tfrm_LOSTE240Q.grd_displayKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tfrm_LOSTE240Q.Copy1Click(Sender: TObject);
var
  select: TGridRect;
  str:String;
  i,j:Integer;
...
procedure Tfrm_LOSTE240Q.grd_displayDrawCell(Sender: TObject; ACol,
...
*******************************************************************************}
procedure Tfrm_LOSTE240Q.StringGrid_DrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; i_align : integer);
var
  LeftPos: Integer;
  TopPos : integer;
  CellStr: string;
...
procedure Tfrm_LOSTE240Q.btn_resetClick(Sender: TObject);
begin
  InitComponent;
end;

procedure Tfrm_LOSTE240Q.InitComponent;
var i : Integer;
begin
