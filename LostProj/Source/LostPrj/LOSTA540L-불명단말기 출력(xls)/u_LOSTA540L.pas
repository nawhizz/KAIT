...
var
  frm_LOSTA540L: Tfrm_LOSTA540L;

implementation

{$R *.DFM}

procedure Tfrm_LOSTA540L.disableComponents;
begin
  disableBtn(Self);
...
procedure Tfrm_LOSTA540L.enableComponents;
begin
  changeBtn(Self);
...
procedure Tfrm_LOSTA540L.initStrGrid;
begin
...
procedure Tfrm_LOSTA540L.setEdtKeyPress;
var i : Integer;
begin
...
procedure Tfrm_LOSTA540L.Edt_onKeyPress ( Sender : TObject; var key : Char);
begin
 if (key = #13) then
   SelectNext( ActiveControl as TEdit , true, True);
end;
{----------------------------------------------------------------------------}
procedure Tfrm_LOSTA540L.FormCreate(Sender: TObject);
begin
   {----------------------- 공통 어플리케이션 설정 ---------------------------}
     setEdtKeyPress;
...
procedure Tfrm_LOSTA540L.btn_CloseClick(Sender: TObject);
begin
     close;
end;

procedure Tfrm_LOSTA540L.dte_FromExit(Sender: TObject);
begin
...
procedure Tfrm_LOSTA540L.dte_ToExit(Sender: TObject);
begin
	sts_Message.Panels[1].Text := '';
...
procedure Tfrm_LOSTA540L.FormShow(Sender: TObject);
begin
  dte_from.SetFocus;
...
procedure Tfrm_LOSTA540L.btn_PrintClick(Sender: TObject);
var
    i:Integer;
...
INQUIRY:
	...
	if (TMAX.SendString('INF003','LOSTA540L') < 0) then  goto LIQUIDATION;
  ...
  //서비스 호출
  if not TMAX.Call('LOSTA540L') then
  begin
  ...
  if count1 < 1 then begin
    ...
    with grd_display do begin
    	for i:=0 to count1-1 do begin
        	...
        	Cells[5,RowPos] := TMAX.RecvString('STR107',i);	// 창고번호
        	Cells[6,RowPos] := TMAX.RecvString('STR108',i);	// 이동전화
        	Cells[7,RowPos] := TMAX.RecvString('STR109',i);	// 보험금상태         

          STR004 := Trim(TMAX.RecvString('STR101',i));
          STR005 := Trim(TMAX.RecvString('STR107',i));
          STR006 := Trim(TMAX.RecvString('STR106',i));
    ...
    //엑셀로 출력
	Proc_gridtoexcel('불명단말기 출력(LOSTA540L)',grd_display.RowCount, grd_display.ColCount, grd_display, 'LOSTA540L');
...
procedure Tfrm_LOSTA540L.btn_ResetClick(Sender: TObject);
begin
	dte_from.Date := date-30;
	dte_to.Date := date;
...
procedure Tfrm_LOSTA540L.btn_queryClick(Sender: TObject);
var
	cmdStr:String;
    filePath:String;
    f:TextFile;
begin
    ...
    filePath:='..\Temp\LOSTA540L_QRY.txt';
