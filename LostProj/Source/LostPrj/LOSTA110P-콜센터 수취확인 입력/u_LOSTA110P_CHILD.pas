...
type
  Tfm_LOSTA110P_CHILD = class(TForm)
    pnl_Command: TPanel;
    Panel1: TPanel;
    Panel2: TPanel;
    rb1: TRadioButton;
...
var
  fm_LOSTA110P_CHILD: Tfm_LOSTA110P_CHILD;

implementation
uses u_LOSTA110P;
{$R *.dfm}

procedure Tfm_LOSTA110P_CHILD.FormCreate(Sender: TObject);
begin
  {----------------------- 공통 어플리케이션 설정 ---------------------------}
...
procedure Tfm_LOSTA110P_CHILD.InitComponent;
var
  i : Integer;
...
begin
  ...
  // 설문에 수정 삭제 기능 삭제 함
  // 2012.01.16 - 수정 기능 다시 추가함
  if (frm_LOSTA110P.lbl_wk_no.Caption <> '0') then
  begin
...
procedure Tfm_LOSTA110P_CHILD.onClick(Sender: TObject);
var
  color : Tcolor;
begin
...
procedure Tfm_LOSTA110P_CHILD.attachOnclickEvt();
var
  i : Integer;
  component : TComponent;
begin
...
procedure Tfm_LOSTA110P_CHILD.btn_CloseClick(Sender: TObject);
begin
  close;
end;

procedure Tfm_LOSTA110P_CHILD.fillCmbTopic(Sender: TObject);
var   i:Integer;
...
begin
  ...
  TMAX.Connect;
	...
	if (TMAX.SendString ('STR001',Trim(frm_LOSTA110P.edt_md_cd.Text     )) < 0) then  goto LIQUIDATION;
	if (TMAX.SendString ('STR002',Trim(frm_LOSTA110P.serial_edit.Text   )) < 0) then  goto LIQUIDATION;
	if (TMAX.SendString ('STR003',delHyphen(frm_LOSTA110P.dte_Ip_Dt.Text)) < 0) then  goto LIQUIDATION;
...
procedure Tfm_LOSTA110P_CHILD.FormShow(Sender: TObject);
begin
  //Self.Show;
  //InitComponent;
  fillCmbTopic(Sender);
...
procedure Tfm_LOSTA110P_CHILD.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  self.Hide;
  frm_LOSTA110P.Enabled := True;
  frm_LOSTA110P.Show;
  frm_LOSTA110P.btn_InquiryClick(Sender);
end;

procedure Tfm_LOSTA110P_CHILD.btn_AddClick(Sender: TObject);
var
  i,j:Integer;
...
begin
  ...
  if (svcNm = 'I01') then
  begin
    STRVALUE[fInc(j)] := frm_LOSTA110P.edt_md_cd.Text;
    STRVALUE[fInc(j)] := frm_LOSTA110P.serial_edit.Text;
    STRVALUE[fInc(j)] := delHyphen(frm_LOSTA110P.dte_Ip_Dt.Text);
  end else
  begin
    STRVALUE[fInc(j)] := Trim(frm_LOSTA110P.lbl_wk_no.Caption);
    STRVALUE[fInc(j)] := frm_LOSTA110P.edt_md_cd.Text;
    STRVALUE[fInc(j)] := frm_LOSTA110P.serial_edit.Text;
    STRVALUE[fInc(j)] := delHyphen(frm_LOSTA110P.dte_Ip_Dt.Text);
  end;
...
procedure Tfm_LOSTA110P_CHILD.btn_UpdateClick(Sender: TObject);
begin
  btn_AddClick(Sender);
end;

procedure Tfm_LOSTA110P_CHILD.btn_DeleteClick(Sender: TObject);
var
  j:Integer;
...
begin
  ...
  STRVALUE[fInc(j)] := Trim(Copy(cmb_topic.Items.Strings[cmb_topic.itemindex],101,10));
  STRVALUE[fInc(j)] := Trim(Copy(cmb_topic.Items.Strings[cmb_topic.itemindex],110, 3));
  STRVALUE[fInc(j)] := frm_LOSTA110P.lbl_wk_no.Caption;
...
procedure Tfm_LOSTA110P_CHILD.cmb_topicChange(Sender: TObject);
var
  i,j:Integer;
