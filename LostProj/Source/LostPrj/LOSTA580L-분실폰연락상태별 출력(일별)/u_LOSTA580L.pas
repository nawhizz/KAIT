{*---------------------------------------------------------------------------
프로그램ID    : LOSTA580L (습득신고자 사은품 발송대상 출력)
프로그램 종류 : Online
작성자	      : 정홍렬
작성일	      : 2011. 08. 30
완료일	      : ####. ##. ##
...
var
  frm_LOSTA580L: Tfrm_LOSTA580L;

implementation
uses u_landprt;
{$R *.DFM}

procedure Tfrm_LOSTA580L.disableComponents;
begin
...
procedure Tfrm_LOSTA580L.enableComponents;
begin
  changeBtn(Self);
...
procedure Tfrm_LOSTA580L.PrtFormShow;
begin
	// 출력 다이로그 박스
...
procedure Tfrm_LOSTA580L.setEdtKeyPress;
var i : Integer;
begin
...
procedure Tfrm_LOSTA580L.Edt_onKeyPress ( Sender : TObject; var key : Char);
begin
 if (key = #13) then
...
{----------------------------------------------------------------------------}
procedure Tfrm_LOSTA580L.FormCreate(Sender: TObject);
begin
   {----------------------- 공통 어플리케이션 설정 ---------------------------}
   setEdtKeyPress;
...
procedure Tfrm_LOSTA580L.btn_CloseClick(Sender: TObject);
begin
     close;
end;

procedure Tfrm_LOSTA580L.dte_fromExit(Sender: TObject);
begin
...
procedure Tfrm_LOSTA580L.dte_toExit(Sender: TObject);
begin
	sts_Message.Panels[1].Text := '';
...
procedure Tfrm_LOSTA580L.btn_PrintClick(Sender: TObject);
var
...
begin
  ...
  TMAX.Connect;
	...
	if (TMAX.SendString('INF003','LOSTA580L') < 0) then  goto LIQUIDATION;
  ...
  serviceSuccess := TMAX.Call('LOSTA580L');
...
procedure Tfrm_LOSTA580L.btn_queryClick(Sender: TObject);
var
	cmdStr:String;
    filePath:String;
    f:TextFile;
begin
    ...
    filePath:='..\Temp\LOSTA580L_QRY.txt';
...
procedure Tfrm_LOSTA580L.FormShow(Sender: TObject);
begin
...
procedure Tfrm_LOSTA580L.btn_ResetClick(Sender: TObject);
begin
	dte_from.Date := date-30;
	dte_to.Date := date;
  cmb_id_cd.ItemIndex := 0;
