...
begin
  ...
  TMAX.Connect;
    ...
    begin
      ...
      (*남        *) Cells[ fInc(j),i+1] := convertWithCommer(  TMAX.RecvString ('INT103',i));
      (*녀        *) Cells[ fInc(j),i+1] := convertWithCommer(  TMAX.RecvString ('INT104',i));
      (*기타      *) Cells[ fInc(j),i+1] := convertWithCommer(  TMAX.RecvString ('INT105',i));
      (*계        *) Cells[ fInc(j),i+1] := convertWithCommer(  TMAX.RecvString ('INT106',i));
      end;
    end;
...
begin
  ...
  with grd_display do begin
    ...
    ColWidths[fInc(j)] := 80;   // 남
    ColWidths[fInc(j)] := 80;   // 녀
