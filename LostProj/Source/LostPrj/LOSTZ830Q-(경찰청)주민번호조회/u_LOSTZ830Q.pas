{*---------------------------------------------------------------------------
프로그램ID    : LOSTZ830Q (주민/사업자/외국인번호 조회)
프로그램 종류 : Online
작성자	      : 유 영 배
작성일	      : 2013. 10. 21
완료일	      : ####. ##. ##
...
procedure Tfrm_LOSTZ830Q.grd_displayDrawCell(Sender: TObject; ACol,
...
procedure Tfrm_LOSTZ830Q.initStrGrid;
begin
...
//모든 콤포넌트 숨기기
procedure Tfrm_LOSTZ830Q.unvisiableComponents;
begin
	//성명
...
//콤포넌트 자리 재 배치
procedure Tfrm_LOSTZ830Q.initComponents;
var
  i : Integer;
	compTop:Integer;
begin
...
procedure Tfrm_LOSTZ830Q.FormCreate(Sender: TObject);
begin
...
procedure Tfrm_LOSTZ830Q.btn_CloseClick(Sender: TObject);
begin
   close;
end;

//'조회' 버튼 클릭
procedure Tfrm_LOSTZ830Q.btn_InquiryClick(Sender: TObject);
var
    i:Integer;
...
INQUIRY:
    ...
    if (TMAX.SendString('INF002',common_username  ) < 0) then  goto LIQUIDATION;
    if (TMAX.SendString('INF002',common_usergroup ) < 0) then  goto LIQUIDATION;
    if (TMAX.SendString('INF003','LOSTZ830Q'      ) < 0) then  goto LIQUIDATION;
    ...
    if (TMAX.SendString('STR005', STR005          ) < 0) then  goto LIQUIDATION;
      ...
      //서비스 호출
    if not TMAX.Call('LOSTZ830Q') then goto LIQUIDATION;
    ...
    with grd_display do begin
      ...
      begin
        ...
        //Cells[ 2,i+1] := InsHyphen(TMAX.RecvString('STR102',i));    // 주민사업자번호
          ...
          Cells[ 3,i+1] :=           TMAX.RecvString('STR104',i);     // 경찰서명
        ...
        //Cells[10,i+1] := TMAX.RecvString('STR112',i);		            // 전화번호
          ...
          Cells[12,i+1] := InsHyphen(TMAX.RecvString('STR114',i));	  // 우편번호
          Cells[13,i+1] := InsHyphen(TMAX.RecvString('STR116',i));	  // 입고일자
          Cells[14,i+1] :=           TMAX.RecvString('STR103',i);     // 기관아이디
...
//On-KeyPress => 스트링 그리드
procedure Tfrm_LOSTZ830Q.grd_displayKeyPress(Sender: TObject; var Key: Char);
begin
	if key = #13 then
...
//'조회조건' 값이 변경된 경우
procedure Tfrm_LOSTZ830Q.search_condition_cbChange(Sender: TObject);
begin
	  //모든 콤포넌트를 숨긴다.
...
procedure Tfrm_LOSTZ830Q.FormShow(Sender: TObject);
var
	str1, str2:String;
begin
  //콤포넌트 자리 재 배치
...
//'선택' 버튼 클릭
procedure Tfrm_LOSTZ830Q.btn_LinkClick(Sender: TObject);
var
  smem:TPSharedMem;
  str_tmp:String;
...
procedure Tfrm_LOSTZ830Q.grd_displayKeyUp(Sender: TObject; var Key: Word;  Shift: TShiftState);
begin
  if key = vk_F2 then
    search_condition_cb.SetFocus;	//'검색조건' 콤보박스로 포커스이동
end;

procedure Tfrm_LOSTZ830Q.FormClose(Sender: TObject;  var Action: TCloseAction);
begin
	//패런트 윈도우를 호출
...
procedure Tfrm_LOSTZ830Q.cmb_model_cbKeyPress(Sender: TObject; var Key: Char);
begin
	if key = #13 then serial_edit.SetFocus;	//시리얼번호로 포커스 이동
end;

//On-KeyPress =>검색조건 콤보박스
procedure Tfrm_LOSTZ830Q.search_condition_cbKeyPress(Sender: TObject;  var Key: Char);
begin
...
*******************************************************************************}
procedure Tfrm_LOSTZ830Q.StringGrid_DrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; i_align : integer);
var
  LeftPos: Integer;
  TopPos : integer;
  CellStr: string;
...
procedure Tfrm_LOSTZ830Q.btn_resetClick(Sender: TObject);
begin
  Self.initComponents;
end;

procedure Tfrm_LOSTZ830Q.onKeyPress(Sender : TObject;var Key : char);
begin
...
procedure Tfrm_LOSTZ830Q.edt_Inq_StrKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then btn_InquiryClick(Sender);
end;

procedure Tfrm_LOSTZ830Q.btn_queryClick(Sender: TObject);
var
	cmdStr    :String;
  filePath  :String;
  f         :TextFile;
