{*---------------------------------------------------------------------------
프로그램ID    : LOSTZ800Q (우편번호 등록)
프로그램 종류 : Online
작성자	      : 정 홍 렬
작성일	      : 2011. 08.16
완료일	      : ####. ##. ##
프로그램 개요 : 우편번호 자료를 조회한다.
...
INQUIRY:
  ...
  with grd_display do begin
    for i:=0 to count1-1 do begin
      Cells[0,RowPos] := TMAX.RecvString('STR101',i);	//성명(업체명)
      Cells[1,RowPos] := TMAX.RecvString('STR102',i);	// 주소
      Cells[2,RowPos] := TMAX.RecvString('STR103',i);	// DDD번호
      Cells[3,RowPos] := TMAX.RecvString('STR104',i);	// 주소2
...
begin
	if not isData then begin 			//스트링 그리드에 데이터가 없으면
    	cmb_Inq_Gu.SetFocus;	//'검색조건' 콤보박스로 이동
      exit;
    end;
  ...
  //공유메모리를 얻는다.
    ...
    begin
        ...
        smem^.ju_so   := Trim(grd_display.Cells[3,grd_display.Row]); // 주소 2
        smem^.ddd_no  := Trim(grd_display.Cells[2,grd_display.Row]); // DDD번호
