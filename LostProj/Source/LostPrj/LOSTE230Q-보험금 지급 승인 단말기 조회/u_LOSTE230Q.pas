...
const
  TITLE   = '보험금 지급 승인 단말기 조회';
  PGM_ID  = 'LOSTE230Q';

type
  Tfrm_LOSTE230Q = class(TForm)
    Bevel1     : TBevel;
    Bevel2     : TBevel;
    dte_to     : TDateEdit;
    dte_from   : TDateEdit;
...
procedure Tfrm_LOSTE230Q.disableComponents;
begin
...
procedure Tfrm_LOSTE230Q.enableComponents;
begin
...
procedure Tfrm_LOSTE230Q.initStrGrid;
begin
	with grd_display do
  begin
    RowCount      :=  2;
    ...
    ColWidths[17] := 110;  // 단말기처리상태
    ColWidths[18] := 100;   // 경찰서습득단말기여부
...
procedure Tfrm_LOSTE230Q.setEdtKeyPress;
var i : Integer;
begin
...
procedure Tfrm_LOSTE230Q.Edt_onKeyPress ( Sender : TObject; var key : Char);
begin
 if (key = #13) then
...
{----------------------------------------------------------------------------}
procedure Tfrm_LOSTE230Q.btn_CloseClick(Sender: TObject);
begin
     close;
end;

procedure Tfrm_LOSTE230Q.FormCreate(Sender: TObject);
begin
  begin
  {----------------------- 공통 어플리케이션 설정 ---------------------------}
     setEdtKeyPress;
...
procedure Tfrm_LOSTE230Q.btn_InquiryClick(Sender: TObject);
var
    // 암호화 관련 변수
...
INQUIRY:
	...
	if (TMAX.SendString('INF003','LOSTE230Q'                                  ) < 0) then  goto LIQUIDATION;
  ...
  if (TMAX.SendString('STR005', Trim(edt_nm.Text)                           ) < 0) then  goto LIQUIDATION;
   ...
   //서비스 호출
   if not TMAX.Call('LOSTE230Q') then
   begin
  ...
  begin
    ...
    begin
      ...
      Cells[17 ,RowPos]   := TMAX.RecvString('STR119',i);
      Cells[18 ,RowPos]   := TMAX.RecvString('STR120',i);
...
procedure Tfrm_LOSTE230Q.FormShow(Sender: TObject);
begin
  InitComponent;
end;

procedure Tfrm_LOSTE230Q.btn_queryClick(Sender: TObject);
var
	cmdStr    :String;
  filePath  :String;
  f:TextFile;
...
procedure Tfrm_LOSTE230Q.btn_excelClick(Sender: TObject);
begin
  Proc_gridtoexcel(TITLE + '(' + PGM_ID + ')',grd_display.RowCount, grd_display.ColCount, grd_display, PGM_ID);
end;

procedure Tfrm_LOSTE230Q.grd_displayKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tfrm_LOSTE230Q.Copy1Click(Sender: TObject);
var
  select: TGridRect;
  str:String;
  i,j:Integer;
...
procedure Tfrm_LOSTE230Q.grd_displayDrawCell(Sender: TObject; ACol,
...
*******************************************************************************}
procedure Tfrm_LOSTE230Q.StringGrid_DrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; i_align : integer);
var
  LeftPos: Integer;
  TopPos : integer;
  CellStr: string;
...
procedure Tfrm_LOSTE230Q.btn_resetClick(Sender: TObject);
begin
  InitComponent;
end;

procedure Tfrm_LOSTE230Q.InitComponent;
var i : Integer;
begin
