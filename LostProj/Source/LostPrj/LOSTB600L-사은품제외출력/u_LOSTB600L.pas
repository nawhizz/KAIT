...
var
  frm_LOSTB600L: Tfrm_LOSTB600L;

implementation
uses u_landprt;
{$R *.DFM}

procedure Tfrm_LOSTB600L.disableComponents;
begin
  disableBtn(Self);
...
procedure Tfrm_LOSTB600L.enableComponents;
begin
  changeBtn(Self);
...
procedure Tfrm_LOSTB600L.PrtFormShow;
begin
	// 출력 다이로그 박스
	frm_LANDPRT := Tfrm_LANDPRT.Create(self);
	frm_LANDPRT.FormSet('Lost600L.txt', lbl_Program_Name.Caption);
	try
...
procedure Tfrm_LOSTB600L.setEdtKeyPress;
var i : Integer;
begin
...
procedure Tfrm_LOSTB600L.Edt_onKeyPress ( Sender : TObject; var key : Char);
begin
 if (key = #13) then
...
{----------------------------------------------------------------------------}
procedure Tfrm_LOSTB600L.FormCreate(Sender: TObject);
begin
  {----------------------- 공통 어플리케이션 설정 ---------------------------}
   setEdtKeyPress;
...
procedure Tfrm_LOSTB600L.btn_CloseClick(Sender: TObject);
begin
     close;
end;

procedure Tfrm_LOSTB600L.dte_fromExit(Sender: TObject);
begin
...
procedure Tfrm_LOSTB600L.dte_toExit(Sender: TObject);
begin
	sts_Message.Panels[1].Text := '';
...
procedure Tfrm_LOSTB600L.btn_PrintClick(Sender: TObject);
var
  count1,i : Integer;
...
begin
  ...
  TMAX.Server := 'KAIT_LOSTPRJ';
	...
	if (TMAX.SendString('INF003','LOSTB600L') < 0) then  goto LIQUIDATION;
  ...
  //서비스 호출
  serviceSuccess := TMAX.Call('LOSTB600L');
...
procedure Tfrm_LOSTB600L.btn_queryClick(Sender: TObject);
var
	cmdStr:String;
    filePath:String;
    f:TextFile;
begin
    ...
    filePath:='..\Temp\LOSTB600L_QRY.txt';
...
procedure Tfrm_LOSTB600L.btn_ResetClick(Sender: TObject);
begin
...
procedure Tfrm_LOSTB600L.FormShow(Sender: TObject);
begin
