{*---------------------------------------------------------------------------
프로그램ID    : LOSTA840Q (경찰서습득단말기 국고귀속 대상 조회)
프로그램 종류 : Online
작성자	      : 유 영 배
작성일	      : 2013. 10. 21
완료일	      : ####. ##. ##
...
var
  frm_LOSTA840Q: Tfrm_LOSTA840Q;


implementation
{$R *.DFM}

procedure Tfrm_LOSTA840Q.disableComponents;
begin
...
procedure Tfrm_LOSTA840Q.enableComponents;
begin
...
procedure Tfrm_LOSTA840Q.initStrGrid;
begin
...
procedure Tfrm_LOSTA840Q.setEdtKeyPress;
var i : Integer;
begin
...
procedure Tfrm_LOSTA840Q.Edt_onKeyPress ( Sender : TObject; var key : Char);
begin
 if (key = #13) then
...
{----------------------------------------------------------------------------}
procedure Tfrm_LOSTA840Q.btn_CloseClick(Sender: TObject);
begin
     close;
end;

procedure Tfrm_LOSTA840Q.FormCreate(Sender: TObject);
begin
  ...
  frm_LOSTA840Q.Position := poScreenCenter;
...
procedure Tfrm_LOSTA840Q.btn_InquiryClick(Sender: TObject);
var
  // 암호화 관련 변수
...
begin
  ...
  //서버에서 메뉴를 가져오기 위해서 TMAX로 연결한다.
	...
	if (TMAX.SendString('INF003','LOSTA840Q') < 0) then  goto LIQUIDATION;
  ...
  //서비스 호출
  if not TMAX.Call('LOSTA840Q') then
  begin
  ...
  if count1 < 1 then begin
    ...
    with grd_display do begin
    	for i:=0 to count1-1 do begin
          ...
          STR002 := delHyphen((TMAX.RecvString('STR105',i)));
          STR003 := delHyphen((TMAX.RecvString('STR106',i)));
          STR004 := Trim(TMAX.RecvString('STR111',i));
          STR005 := Trim(TMAX.RecvString('STR102',i));
          STR006 := Trim(TMAX.RecvString('STR104',i));
          ...
          Cells[13,RowPos] := ECPlazaSeed.Decrypt(seed_gano, common_seedkey);
          seed_mtno        := TMAX.RecvString('STR114',i);
          Cells[14,RowPos] := ECPlazaSeed.Decrypt(seed_mtno, common_seedkey);
...
procedure Tfrm_LOSTA840Q.dte_fromExit(Sender: TObject);
begin
 sts_Message.Panels[1].Text := '';
end;

procedure Tfrm_LOSTA840Q.FormShow(Sender: TObject);
begin
  btn_resetClick(Sender);
end;

procedure Tfrm_LOSTA840Q.grd_displayDrawCell(Sender: TObject; ACol,
...
procedure Tfrm_LOSTA840Q.grd_displayKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tfrm_LOSTA840Q.Copy1Click(Sender: TObject);
var
  select: TGridRect;
  str:String;
  i,j:Integer;
...
procedure Tfrm_LOSTA840Q.btn_queryClick(Sender: TObject);
var
	cmdStr:String;
    filePath:String;
    f:TextFile;
begin
    ...
    filePath:='..\Temp\LOSTA840Q_QRY.txt';
...
procedure Tfrm_LOSTA840Q.btn_excelClick(Sender: TObject);
begin
	Proc_gridtoexcel('경찰서습득단말기국고귀속대상조회(LOSTA840Q)',grd_display.RowCount, grd_display.ColCount, grd_display, 'LOSTA840Q');
end;
...
*******************************************************************************}
procedure Tfrm_LOSTA840Q.StringGrid_DrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; i_align : integer);
var
  LeftPos: Integer;
  TopPos : integer;
  CellStr: string;
...
procedure Tfrm_LOSTA840Q.dte_fromKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tfrm_LOSTA840Q.btn_resetClick(Sender: TObject);
var i : Integer;
begin
  changeBtn(Self);
