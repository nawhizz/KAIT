{*---------------------------------------------------------------------------
프로그램ID    : LOSTA340Q (기간별 단말기 처리 현황)
프로그램 종류 : Online
작성자	      : 최 대 성
작성일	      : 2011. 08. 10
완료일	      : ####. ##. ##
...
uses
  ...
  Menus, WinSkinData, so_tmax, common_lib, LOSTA340Q_PRT_HEAD,LOSTA340Q_PRT_HEAD2, Clipbrd, ComObj;
...
type
  Tfrm_LOSTA340Q = class(TForm)
    ...
    procedure Cmb_id_chg(Sender: TObject);
    procedure btn_CloseClick(Sender: TObject);

    function chkToStr(boolean : bool) : String;
...
var
  frm_LOSTA340Q: Tfrm_LOSTA340Q;
  arrIdCd : TZ0xxArray;  

implementation
uses cpaklibm, StdConvs;
{$R *.DFM}

procedure Tfrm_LOSTA340Q.setEdtKeyPress;
var i : Integer;
begin
...
procedure Tfrm_LOSTA340Q.Edt_onKeyPress ( Sender : TObject; var key : Char);
begin
 if (key = #13) then
   SelectNext( ActiveControl as TEdit , true, True);
end;

procedure Tfrm_LOSTA340Q.FormCreate(Sender: TObject);
begin
...
*******************************************************************************}
procedure Tfrm_LOSTA340Q.initStrGrid;
...
procedure Tfrm_LOSTA340Q.btn_InquiryClick(Sender: TObject);
var
...
INQUIRY:
    ...
    if (copy(cmb_gubun.Items.Strings[cmb_gubun.ItemIndex],41,1) = 'T') then svcNm := 'S01'
    else svcNm := 'S02';
    ...
    if (TMAX.SendString('INF003','LostA340Q')     < 0)  then  goto LIQUIDATION;
    ...
    if (TMAX.SendString('STR003', copy(cmb_gubun.Items[cmb_gubun.ItemIndex],41,1))< 0) then  goto LIQUIDATION;
    if (TMAX.SendString('STR004', arrIdCd[cmb_id_cd.ItemIndex].code) < 0) then  goto LIQUIDATION;
    if (TMAX.SendString('STR005', chkToStr(chk_sm_yn.checked)) < 0) then  goto LIQUIDATION;
    if (TMAX.SendString('STR006', chkToStr(chk_pd_yn.checked)) < 0) then  goto LIQUIDATION;

    // 사용자 데이터 설정 완료

    //서비스 호출
    if not TMAX.Call('LOSTA340Q') then
    begin
        ...
        begin
          if(svcNm = 'S01') then
            begin
              ...
              Cells[3,RowPos]   := convertWithCommer(TMAX.RecvString('INT103',i)); // 사업자전달
              Cells[4,RowPos]   := convertWithCommer(TMAX.RecvString('INT104',i)); // 보험사매각(협회)
              Cells[5,RowPos]   := convertWithCommer(TMAX.RecvString('INT105',i)); // 보험사매각(경찰)
            end
          else
            begin
              Cells[0,RowPos]   := TMAX.RecvString('STR101',i); // 사업자
              Cells[1,RowPos]   := TMAX.RecvString('STR102',i); // 사업자
              Cells[2,RowPos]   := convertWithCommer(TMAX.RecvString('INT101',i)); // 입고
              Cells[3,RowPos]   := convertWithCommer(TMAX.RecvString('INT102',i)); // 출고
              Cells[4,RowPos]   := convertWithCommer(TMAX.RecvString('INT103',i)); // 장기/귀속
              Cells[5,RowPos]   := convertWithCommer(TMAX.RecvString('INT104',i)); // 보험사매각(협회)
              Cells[6,RowPos]   := convertWithCommer(TMAX.RecvString('INT105',i)); // 보험사매각(경찰)
            end;
          Inc(RowPos);
        end;
    end;
...
procedure Tfrm_LOSTA340Q.FormShow(Sender: TObject);
begin
...
*******************************************************************************}
procedure Tfrm_LOSTA340Q.btn_PrintClick(Sender: TObject);
var
    head:TLOSTA340Q_PRT_HEAD;	//head 계산
    head2:TLOSTA340Q_PRT_HEAD2;	//head 계산
    Canvas: TCanvas;
    i, j, page : integer;
    ...
    var
      ...
      str : String;
    begin
      centerPrint(prntWidth div 2,  	 '기간별 단말기 처리현황');
      //상기 문자열에 대한 밑줄 긋기
      centerUnderLine(prntWidth div 2, '기간별 단말기 처리현황');

      //머릿말 시작------------------------------------
      Inc(curntYposi, -sheight);
      Inc(curntYposi, -ygap);

      str := 'PROG ID: LOSTA340Q';
      ...
      begin
        for i:=0 to 5 do
          rightPrint(head.getRightPosition(i), titles1[i]);
      end
      else
      begin
        for i:=0 to 6 do
          rightPrint(head2.getRightPosition(i), titles2[i]);
      end;
...
begin
    ...
    if(cmb_gubun.ItemIndex = 0) then Printer.Orientation := poLandscape
    ...
    begin
      prntWidth := 2090;	//A4 = 297mm
      prntHeight:= 2960;	//A4 = 210mm
      prntMargin:= 200;	  //left, right margin = 20mm
    end;
    ...
    //위치 중요
    if(cmb_gubun.ItemIndex = 0) then
      head := TLOSTA340Q_PRT_HEAD.Create(prntMargin, prntWidth, Canvas)   //마진, 전체폭,...
    else
      head2 := TLOSTA340Q_PRT_HEAD2.Create(prntMargin, prntWidth, Canvas);  //마진, 전체폭,...
    ...
    begin
      ...
      lineEnd     := head.getRightPosition(5) +20						//선긋기 x-축 끝점
    end
    else
    begin
      lineStart   := head2.getRightPosition(0)- head2.getLength(0) -20;	//선긋기 x-축 시작점
      lineEnd     := head2.getRightPosition(6) +20;						//선긋기 x-축 끝점
    end;
    //타이틀 프린트
    printTitle(page);
    ...
    with grd_display do begin
    	for j:= 1 to RowCount-2 do begin
            ...
            if(cmb_gubun.ItemIndex = 0) then
              ...
              rightPrint(head2.getRightPosition(i), Cells[i,j]);

      Inc(curntYposi, -sheight);	//다음줄 이동
      Inc(curntYposi, -ygap);		//1mm 아래로 이동

			//출력물이 페이지를 넘어가면...
          if curntYposi*-1 + 140 >= prntHeight then begin
    ...
    for i:= 0 to grd_display.ColCount-1 do
      if (cmb_gubun.ItemIndex = 0) then
        ...
        rightPrint(head2.getRightPosition(i), grd_display.Cells[i,grd_display.RowCount-1]);
    ...
    if(cmb_gubun.ItemIndex = 0) then head.Free 	  //헤드 정보 삭제
    else head2.Free; 	//헤드 정보 삭제
...
*******************************************************************************}
procedure Tfrm_LOSTA340Q.disableComponents;
begin
  disableBtn(Self);
...
*******************************************************************************}
procedure Tfrm_LOSTA340Q.enableComponents;
begin
  changeBtn(Self);
...
*
*******************************************************************************}
procedure Tfrm_LOSTA340Q.grd_displayDrawCell(Sender: TObject; ACol,
  ARow: Integer; Rect: TRect; State: TGridDrawState);
var
  grid:TStringGrid;
  S:String;
begin
  ...
  if (ARow = 0) then begin
    ...
    begin
      if(copy(cmb_gubun.Items.Strings[cmb_gubun.ItemIndex],41,1) = 'T') then
        begin
          case ACol of
...
*******************************************************************************}
procedure Tfrm_LOSTA340Q.grd_displayKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
*******************************************************************************}
procedure Tfrm_LOSTA340Q.Copy1Click(Sender: TObject);
var
  select: TGridRect;
  str:String;
  i,j:Integer;
...
*******************************************************************************}
procedure Tfrm_LOSTA340Q.StringGrid_DrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; i_align : integer);
var
  LeftPos: Integer;
  TopPos : integer;
  CellStr: string;
...
procedure Tfrm_LOSTA340Q.Cmb_id_chg(Sender: TObject);
begin
  initStrGrid;
end;

procedure Tfrm_LOSTA340Q.btn_CloseClick(Sender: TObject);
begin
  self.Close;
end;

function Tfrm_LOSTA340Q.chkToStr(boolean : bool) : String;
begin
  if(boolean) then result := '1'
  else result := '0'
  end;
procedure Tfrm_LOSTA340Q.btn_resetClick(Sender: TObject);
var i : Integer;
begin
  // 버튼 이미지 초기화
  changeBtn(Self);
...
procedure Tfrm_LOSTA340Q.btn_queryClick(Sender: TObject);
var
	cmdStr:String;
    filePath:String;
    f:TextFile;
begin
    ...
    filePath:='..\Temp\LOSTA340Q_QRY.txt';
