{*---------------------------------------------------------------------------
프로그램ID    : lostp06l (분실폰 수취확인 대상 출력)
프로그램 종류 : Online
작성자	      : 정 홍렬
작성일	      : 2011. 08. 23
완료일	      : ####. ##. ##
...
var
  frm_LOSTA510L: Tfrm_LOSTA510L;

implementation
uses u_landprt;
{$R *.DFM}

procedure Tfrm_LOSTA510L.disableComponents;
begin
...
procedure Tfrm_LOSTA510L.enableComponents;
begin
...
procedure Tfrm_LOSTA510L.PrtFormShow;
begin
	// 출력 다이로그 박스
	frm_LANDPRT := Tfrm_LANDPRT.Create(self);
	frm_LANDPRT.FormSet('LostA510L.txt', lbl_Program_Name.Caption);
	try
...
procedure Tfrm_LOSTA510L.setEdtKeyPress;
var i : Integer;
begin
...
procedure Tfrm_LOSTA510L.Edt_onKeyPress ( Sender : TObject; var key : Char);
begin
 if (key = #13) then
...
{----------------------------------------------------------------------------}
procedure Tfrm_LOSTA510L.FormCreate(Sender: TObject);
begin
...
procedure Tfrm_LOSTA510L.btn_CloseClick(Sender: TObject);
begin
     close;
end;

procedure Tfrm_LOSTA510L.dte_fromExit(Sender: TObject);
begin
...
procedure Tfrm_LOSTA510L.dte_toExit(Sender: TObject);
begin
	sts_Message.Panels[1].Text := '';
...
procedure Tfrm_LOSTA510L.btn_PrintClick(Sender: TObject);
var
  count1,i : Integer;
...
begin
  ...
  TMAX.Connect;
	...
	if (TMAX.SendString('INF003','LOSTA510L') < 0) then  goto LIQUIDATION;
  ...
  serviceSuccess := TMAX.Call('LOSTA510L');
...
procedure Tfrm_LOSTA510L.FormShow(Sender: TObject);
begin
  dte_from.Date := date;
  dte_to.Date := date;
end;

procedure Tfrm_LOSTA510L.btn_resetClick(Sender: TObject);
begin
  dte_from.Date := date;
  dte_to.Date := date;
  changeBtn(Self);
...
procedure Tfrm_LOSTA510L.btn_queryClick(Sender: TObject);
var
	cmdStr:String;
    filePath:String;
    f:TextFile;
