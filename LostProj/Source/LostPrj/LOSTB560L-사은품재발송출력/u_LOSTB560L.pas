...
procedure Tfrm_LOSTB560L.disableComponents;
begin
...
procedure Tfrm_LOSTB560L.enableComponents;
begin
  changeBtn(Self);
...
procedure Tfrm_LOSTB560L.PrtFormShow;
begin
	// 출력 다이로그 박스
	frm_LANDPRT := Tfrm_LANDPRT.Create(self);
	frm_LANDPRT.FormSet('LostB560L.txt', lbl_Program_Name.Caption);
	try
...
procedure Tfrm_LOSTB560L.setEdtKeyPress;
var i : Integer;
begin
...
procedure Tfrm_LOSTB560L.Edt_onKeyPress ( Sender : TObject; var key : Char);
begin
 if (key = #13) then
   SelectNext( ActiveControl as TEdit , true, True);
end;
{----------------------------------------------------------------------------}
procedure Tfrm_LOSTB560L.FormCreate(Sender: TObject);
begin
  {----------------------- 공통 어플리케이션 설정 ---------------------------}
   setEdtKeyPress;
...
procedure Tfrm_LOSTB560L.btn_CloseClick(Sender: TObject);
begin
  close;
end;

procedure Tfrm_LOSTB560L.dte_fromExit(Sender: TObject);
begin
...
procedure Tfrm_LOSTB560L.dte_toExit(Sender: TObject);
begin
	sts_Message.Panels[1].Text := '';
...
procedure Tfrm_LOSTB560L.btn_PrintClick(Sender: TObject);
var
  count1,i : Integer;
...
begin
  ...
  TMAX.Connect;
	...
	if (TMAX.SendString('INF003','LOSTB560L') < 0) then  goto LIQUIDATION;
  ...
  //서비스 호출
  serviceSuccess := TMAX.Call('LOSTB560L');
...
procedure Tfrm_LOSTB560L.FormShow(Sender: TObject);
begin
  btn_ResetClick(Sender);
end;

procedure Tfrm_LOSTB560L.btn_ResetClick(Sender: TObject);
begin
...
procedure Tfrm_LOSTB560L.btn_queryClick(Sender: TObject);
var
	cmdStr:String;
    filePath:String;
    f:TextFile;
begin
    ...
    filePath:='..\Temp\LOSTB560L_QRY.txt';
