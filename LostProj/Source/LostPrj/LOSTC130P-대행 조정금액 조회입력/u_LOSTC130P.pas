...
uses
  ...
  so_tmax, WinSkinData,u_LOSTC130P_CHILD, monthEdit, ComObj;
...
var
  ...
  frm_LOSTC130P: Tfrm_LOSTC130P;

implementation

{$R *.dfm}

procedure Tfrm_LOSTC130P.initStrGrid;
begin
...
procedure Tfrm_LOSTC130P.SetDate;
var
  // 서비스에서 추가로 받는 정보 변수
  DU_DT : String;
  AC_DT : String;
...
INQUIRY:
	...
	if (TMAX.SendString('INF003','LOSTC130P') < 0) then  goto LIQUIDATION;
  ...
  if (TMAX.SendString('STR001', CalendarMonth1.Text) < 0) then  goto LIQUIDATION;
    ...
    //서비스 호출
	if not TMAX.Call('LOSTC130P') then
  begin
    ...
    DU_DT := TMAX.RecvString('STR101',0);
    AC_DT := TMAX.RecvString('STR102',0);
    CT_01 := TMAX.RecvString('STR103',0); 
    AC_GU := TMAX.RecvString('STR104',0); 
    FT_DT := TMAX.RecvString('STR105',0); 
    TO_DT := TMAX.RecvString('STR106',0);
...
*******************************************************************************}
procedure Tfrm_LOSTC130P.StringGrid_DrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; i_align : integer);
var
  LeftPos: Integer;
  TopPos : integer;
  CellStr: string;
...
procedure Tfrm_LOSTC130P.setEdtKeyPress;
var i : Integer;
begin
...
procedure Tfrm_LOSTC130P.Edt_onKeyPress ( Sender : TObject; var key : Char);
begin
 if (key = #13) then
   SelectNext( ActiveControl as TEdit , true, True);
end;

procedure Tfrm_LOSTC130P.disableComponents;
begin
...
procedure Tfrm_LOSTC130P.enableComponents;
begin
...
{----------------------------------------------------------------------------}
procedure Tfrm_LOSTC130P.FormCreate(Sender: TObject);
begin
...
procedure Tfrm_LOSTC130P.btn_CloseClick(Sender: TObject);
begin
  close;
end;

procedure Tfrm_LOSTC130P.btn_InquiryClick(Sender: TObject);
var
  i:Integer;
...
INQUIRY:
  ...
  with grd_display do begin
    for i:=0 to count1-1 do begin
      ...
      Cells[3,RowPos] := TMAX.RecvString('INT103',i);  //우체국미등록건수
      Cells[4,RowPos] := convertWithCommer(IntToStr(TMAX.RecvInteger('INT104',i)));  //조정금액
      Cells[5,RowPos] := TMAX.RecvString('STR105',i);  //조정내역
      Inc(seq);
      Inc(RowPos);
    end;
  end;
...
procedure Tfrm_LOSTC130P.btn_DeleteClick(Sender: TObject);
...
procedure Tfrm_LOSTC130P.grd_displayDblClick(Sender: TObject);
begin
  ...
  frm_LOSTC130P_CHILD.FormShow(Sender);
end;

procedure Tfrm_LOSTC130P.ct_ymKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tfrm_LOSTC130P.btn_AddClick(Sender: TObject);
var
...
begin
  ...
  if (TMAX.SendString('STR003', delHyphen(dte_to.Text)    )   < 0) then  goto LIQUIDATION;
    ...
    with grd_display do begin
        ...
        if (TMAX.SendString ('STR011', Trim(Cells[1,RowPos])  )   < 0) then  goto LIQUIDATION;
        ...
        if (TMAX.SendString ('STR014', Trim(Cells[5,RowPos])  )   < 0) then  goto LIQUIDATION;
        Inc(RowPos);
      end;
    end;
	if not TMAX.Call('LOSTC130P') then
...
procedure Tfrm_LOSTC130P.btn_UpdateClick(Sender: TObject);
var
  i,seq,RowPos:Integer;
LABEL LIQUIDATION;
begin
...
procedure Tfrm_LOSTC130P.CalendarMonth1Change(Sender: TObject);
begin
  SetDate;
end;

procedure Tfrm_LOSTC130P.FormShow(Sender: TObject);
begin
  btn_resetClick(Sender);
end;

procedure Tfrm_LOSTC130P.grd_displayDrawCell(Sender: TObject; ACol,
...
procedure Tfrm_LOSTC130P.btn_resetClick(Sender: TObject);
var i : Integer;
begin
  changeBtn(Self);
...
procedure Tfrm_LOSTC130P.btn_queryClick(Sender: TObject);
var
	cmdStr:String;
    filePath:String;
    f:TextFile;
begin
    ...
    filePath:='..\Temp\LOSTC130_QRY.txt';
...
procedure Tfrm_LOSTC130P.CalendarMonth1KeyPress(Sender: TObject;
  var Key: Char);
begin
 key := #0;
end;
