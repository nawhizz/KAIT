...
var
  frm_LOSTD540L: Tfrm_LOSTD540L;

implementation
uses u_landprt;
{$R *.DFM}

procedure Tfrm_LOSTD540L.disableComponents;
begin
...
procedure Tfrm_LOSTD540L.enableComponents;
begin
  changeBtn(Self);
...
procedure Tfrm_LOSTD540L.setEdtKeyPress;
var i : Integer;
begin
...
procedure Tfrm_LOSTD540L.Edt_onKeyPress ( Sender : TObject; var key : Char);
begin
 if (key = #13) then
   SelectNext( ActiveControl as TEdit , true, True);
end;

procedure Tfrm_LOSTD540L.PrtFormShow;
begin
	// 출력 다이로그 박스
	frm_LANDPRT := Tfrm_LANDPRT.Create(self);
	frm_LANDPRT.FormSet('LostD540L.txt', lbl_Program_Name.Caption);
	try
...
{----------------------------------------------------------------------------}
procedure Tfrm_LOSTD540L.FormCreate(Sender: TObject);
begin
  {----------------------- 공통 어플리케이션 설정 ---------------------------}
   setEdtKeyPress;
...
procedure Tfrm_LOSTD540L.btn_CloseClick(Sender: TObject);
begin
     close;
end;

procedure Tfrm_LOSTD540L.dte_fromExit(Sender: TObject);
begin
...
procedure Tfrm_LOSTD540L.dte_toExit(Sender: TObject);
begin
	sts_Message.Panels[1].Text := '';
...
procedure Tfrm_LOSTD540L.btn_PrintClick(Sender: TObject);
var
  count1,i : Integer;
...
begin
  ...
  TMAX.Connect;
	...
	if (TMAX.SendString('INF003','LOSTD540L') < 0) then  goto LIQUIDATION;
  ...
  //서비스 호출
  serviceSuccess := TMAX.Call('LOSTD540L');
...
procedure Tfrm_LOSTD540L.btn_ResetClick(Sender: TObject);
begin
	dte_from.Date := date;
	dte_to.Date := date;
...
procedure Tfrm_LOSTD540L.FormShow(Sender: TObject);
begin
  btn_ResetClick(Sender);
end;

procedure Tfrm_LOSTD540L.btn_queryClick(Sender: TObject);
var
	cmdStr:String;
    filePath:String;
    f:TextFile;
begin
    ...
    filePath:='..\Temp\LOSTD540_QRY.txt';
