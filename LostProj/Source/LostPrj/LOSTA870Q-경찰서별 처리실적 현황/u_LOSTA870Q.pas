...
begin
    ...
    cells[fInc(j),0] := '지방경찰청기관아이디';
    cells[fInc(j),0] := '지방경찰청명';
    cells[fInc(j),0] := '경찰서기관아이디';
    cells[fInc(j),0] := '경찰서명';
    cells[fInc(j),0] := '습득건수';
    cells[fInc(j),0] := '입고건수';
    ...
    cells[fInc(j),0] := '출고건수';
    cells[fInc(j),0] := '국가귀속건수';
    cells[fInc(j),0] := '합계';

    j := 0;

    colwidths[fInc(j)] :=  -1;   // 지방경찰청기관아이디
    colwidths[fInc(j)] := 160;   // 지방경찰청명
    colwidths[fInc(j)] :=  -1;   // 경찰서기관아이디
    colwidths[fInc(j)] := 160;   // 경찰서명
    colwidths[fInc(j)] := 100;   // 습득건수
    colwidths[fInc(j)] := 100;   // 입고건수
    colwidths[fInc(j)] := 100;   // 상담중건수
    colwidths[fInc(j)] := 100;   // 수취확인건수
    colwidths[fInc(j)] := 100;   // 출고건수
    colwidths[fInc(j)] := 100;   // 국가귀속건수
...
begin
    ...
    with grd_display do begin
        ...
        begin
          ...
          Cells[ 0,i+1]   :=                   TMAX.RecvString('STR101',i);   // 기관아이디(경찰청)
          Cells[ 1,i+1]   :=                   TMAX.RecvString('STR102',i);   // 지방경찰청명
          Cells[ 2,i+1]   :=                   TMAX.RecvString('STR103',i);   // 기관아이디(경찰서)
          Cells[ 3,i+1]   :=                   TMAX.RecvString('STR104',i);   // 경찰서명
          Cells[ 4,i+1]   := convertWithCommer(TMAX.RecvString('INT105',i));  // 습득건수
          Cells[ 5,i+1]   := convertWithCommer(TMAX.RecvString('INT106',i));  // 입고건수
          Cells[ 6,i+1]   := convertWithCommer(TMAX.RecvString('INT107',i));  // 상담중건수
          Cells[ 7,i+1]   := convertWithCommer(TMAX.RecvString('INT108',i));  // 수취확인건수
          Cells[ 8,i+1]   := convertWithCommer(TMAX.RecvString('INT109',i));  // 출고건수
          Cells[ 9,i+1]   := convertWithCommer(TMAX.RecvString('INT110',i));  // 국가귀속건수
          Cells[10,i+1]   := convertWithCommer(TMAX.RecvString('INT111',i));  // 합계
...
begin
	Proc_gridtoexcel('경찰서별 처리실적 현황(LOSTA870Q)',grd_display.RowCount, grd_display.ColCount, grd_display, 'LOSTB270Q');
end;
