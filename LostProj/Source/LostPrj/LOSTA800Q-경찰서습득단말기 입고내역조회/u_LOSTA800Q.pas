{*---------------------------------------------------------------------------
프로그램ID    : LOSTA800Q (경찰서습득단말기 입고 내역 조회)
프로그램 종류 : Online
작성자	      : 유 영 배
작성일	      : 2013. 10. 21
완료일	      : ####. ##. ##
...
var
  frm_LOSTA800Q: Tfrm_LOSTA800Q;


implementation
{$R *.DFM}

procedure Tfrm_LOSTA800Q.disableComponents;
begin
...
procedure Tfrm_LOSTA800Q.enableComponents;
begin
...
procedure Tfrm_LOSTA800Q.initStrGrid;
begin
...
procedure Tfrm_LOSTA800Q.setEdtKeyPress;
var i : Integer;
begin
...
procedure Tfrm_LOSTA800Q.Edt_onKeyPress ( Sender : TObject; var key : Char);
begin
 if (key = #13) then
...
{----------------------------------------------------------------------------}
procedure Tfrm_LOSTA800Q.btn_CloseClick(Sender: TObject);
begin
     close;
end;

procedure Tfrm_LOSTA800Q.FormCreate(Sender: TObject);
begin
  ...
  frm_LOSTA800Q.Position := poScreenCenter;
...
procedure Tfrm_LOSTA800Q.btn_InquiryClick(Sender: TObject);
var
  i:Integer;
...
begin
  ...
  //서버에서 메뉴를 가져오기 위해서 TMAX로 연결한다.
	...
	if (TMAX.SendString('INF003','LOSTA800Q') < 0) then  goto LIQUIDATION;
  ...
  //서비스 호출
  if not TMAX.Call('LOSTA800Q') then
  begin
  ...
  if count1 < 1 then begin
    ...
    with grd_display do begin
        ...
        //Cells[13,RowPos] := TMAX.RecvString('STR113',i);
          seed_name        := TMAX.RecvString('STR113',i);     // 가입자성명
          Cells[13,RowPos] := ECPlazaSeed.Decrypt(seed_name, common_seedkey);
        //Cells[14,RowPos] := InsHyphen(TMAX.RecvString('STR114',i));
          seed_idno        := TMAX.RecvString('STR114',i);     // 가입자주민번호
          Cells[14,RowPos] := InsHyphen(ECPlazaSeed.Decrypt(seed_idno, common_seedkey));
        //Cells[15,RowPos] := TMAX.RecvString('STR115',i);
          seed_tlno        := TMAX.RecvString('STR115',i);     // 가입자연락처
          Cells[15,RowPos] := ECPlazaSeed.Decrypt(seed_tlno, common_seedkey);
        //Cells[16,RowPos] := TMAX.RecvString('STR116',i);
          seed_mtno        := TMAX.RecvString('STR116',i);     // 분실핸드폰번호
          Cells[16,RowPos] := ECPlazaSeed.Decrypt(seed_mtno, common_seedkey);
...
procedure Tfrm_LOSTA800Q.dte_fromExit(Sender: TObject);
begin
...
procedure Tfrm_LOSTA800Q.dte_toExit(Sender: TObject);
begin
     sts_Message.Panels[1].Text := '';
...
procedure Tfrm_LOSTA800Q.FormShow(Sender: TObject);
begin
  btn_resetClick(Sender);
end;

procedure Tfrm_LOSTA800Q.grd_displayDrawCell(Sender: TObject; ACol,
...
procedure Tfrm_LOSTA800Q.grd_displayKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tfrm_LOSTA800Q.Copy1Click(Sender: TObject);
var
  select: TGridRect;
  str:String;
  i,j:Integer;
...
procedure Tfrm_LOSTA800Q.btn_queryClick(Sender: TObject);
var
	cmdStr:String;
    filePath:String;
    f:TextFile;
begin
    ...
    filePath:='..\Temp\LOSTA800Q_QRY.txt';
...
procedure Tfrm_LOSTA800Q.btn_excelClick(Sender: TObject);
begin
	Proc_gridtoexcel('경찰서습득단말기입고내역조회(LOSTA800Q)',grd_display.RowCount, grd_display.ColCount, grd_display, 'LOSTA800Q');
end;
...
*******************************************************************************}
procedure Tfrm_LOSTA800Q.StringGrid_DrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; i_align : integer);
var
  LeftPos: Integer;
  TopPos : integer;
  CellStr: string;
...
procedure Tfrm_LOSTA800Q.dte_fromKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tfrm_LOSTA800Q.dte_toKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if Key = VK_RETURN then
      cmb_id_cd.SetFocus;
...
procedure Tfrm_LOSTA800Q.cmb_id_cdKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if Key = VK_RETURN then
...
procedure Tfrm_LOSTA800Q.btn_resetClick(Sender: TObject);
var i : Integer;
begin
  changeBtn(Self);
