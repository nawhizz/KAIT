{*---------------------------------------------------------------------------
프로그램ID    : LOSTA560L (분실폰 수취확인 대상 출력)
프로그램 종류 : Online
작성자	      : 정 홍 렬
작성일	      : 2011. 08. 19
완료일	      : ####. ##. ##
...
var
  frm_LOSTA560L: Tfrm_LOSTA560L;

implementation
uses u_landprt;
{$R *.DFM}

procedure Tfrm_LOSTA560L.disableComponents;
begin
...
procedure Tfrm_LOSTA560L.enableComponents;
begin
  changeBtn(Self);
...
procedure Tfrm_LOSTA560L.PrtFormShow;
begin
	// 출력 다이로그 박스
...
procedure Tfrm_LOSTA560L.setEdtKeyPress;
 var i : Integer;
     edt : TEdit;
 begin
...
procedure Tfrm_LOSTA560L.Edt_onKeyPress ( Sender : TObject; var key : Char);
 begin
   if (key = #13) then
...
{----------------------------------------------------------------------------}
procedure Tfrm_LOSTA560L.FormCreate(Sender: TObject);
begin
  ...
  initComboBoxWithZ0xx('LOSTA560L.dat', cmb_sts_gu_d, '전체', ' ',cmb_sts);
...
procedure Tfrm_LOSTA560L.btn_CloseClick(Sender: TObject);
begin
     close;
...
procedure Tfrm_LOSTA560L.dte_fromExit(Sender: TObject);
begin
...
procedure Tfrm_LOSTA560L.dte_toExit(Sender: TObject);
begin
	sts_Message.Panels[1].Text := '';
...
procedure Tfrm_LOSTA560L.btn_PrintClick(Sender: TObject);
var
...
begin
  ...
  TMAX.Connect;
	...
	if (TMAX.SendString('INF003','LOSTA560L') < 0) then  goto LIQUIDATION;
  ...
  if (TMAX.SendString('STR004', cmb_sts_gu_d[cmb_sts.ItemIndex].code) < 0) then  goto LIQUIDATION;
  if (TMAX.SendString('STR005', cmb_sts.Text) < 0) then  goto LIQUIDATION;

  serviceSuccess := TMAX.Call('LOSTA560L');
...
procedure Tfrm_LOSTA560L.btn_ResetClick(Sender: TObject);
begin
...
procedure Tfrm_LOSTA560L.btn_queryClick(Sender: TObject);
var
	cmdStr:String;
    filePath:String;
    f:TextFile;
...
procedure Tfrm_LOSTA560L.FormShow(Sender: TObject);
begin
