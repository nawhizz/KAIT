...
var
  frm_LOSTA210Q: Tfrm_LOSTA210Q;


implementation
{$R *.DFM}

procedure Tfrm_LOSTA210Q.disableComponents;
begin
...
procedure Tfrm_LOSTA210Q.enableComponents;
begin
...
procedure Tfrm_LOSTA210Q.initStrGrid;
begin
...
procedure Tfrm_LOSTA210Q.setEdtKeyPress;
var i : Integer;
begin
...
procedure Tfrm_LOSTA210Q.Edt_onKeyPress ( Sender : TObject; var key : Char);
begin
 if (key = #13) then
...
{----------------------------------------------------------------------------}
procedure Tfrm_LOSTA210Q.btn_CloseClick(Sender: TObject);
begin
     close;
end;

procedure Tfrm_LOSTA210Q.FormCreate(Sender: TObject);
begin
  ...
  frm_LOSTA210Q.Position := poScreenCenter;
...
procedure Tfrm_LOSTA210Q.btn_InquiryClick(Sender: TObject);
var
  i:Integer;
...
begin
  ...
  //서버에서 메뉴를 가져오기 위해서 TMAX로 연결한다.
	...
	if (TMAX.SendString('INF003','LOSTA210Q') < 0) then  goto LIQUIDATION;
  ...
  if (TMAX.SendString('STR003', cmb_id_cd_d[cmb_id_cd.ItemIndex].code) < 0) then  goto LIQUIDATION;
    ...
    //서비스 호출
	if not TMAX.Call('LOSTA210Q') then goto LIQUIDATION;
...
STR008 : 조회시작 일련번호
  ...
  //서비스 호출
  if not TMAX.Call('LOSTA210Q') then
  begin
  ...
  if count1 < 1 then begin
    ...
    with grd_display do begin
    	for i:=0 to count1-1 do begin
          ...
          STR007 := Trim(TMAX.RecvString('STR103',i));
          STR008 := Trim(TMAX.RecvString('STR105',i));
        ...
        //Cells[7,RowPos]  := TMAX.RecvString('STR107',i);
          seed_name        := TMAX.RecvString('STR107',i);
          Cells[7,RowPos]  := ECPlazaSeed.Decrypt(seed_name, common_seedkey);
        //Cells[8,RowPos]  := InsHyphen(TMAX.RecvString('STR108',i));
          seed_idno        := TMAX.RecvString('STR108',i);
          Cells[8,RowPos]  := ECPlazaSeed.Decrypt(seed_idno, common_seedkey);
        //Cells[9,RowPos]  := TMAX.RecvString('STR109',i);
          seed_tlno        := TMAX.RecvString('STR109',i);
          Cells[9,RowPos]  := ECPlazaSeed.Decrypt(seed_tlno, common_seedkey);
        //Cells[10,RowPos] := TMAX.RecvString('STR110',i);
          seed_mtno        := TMAX.RecvString('STR110',i);
...
procedure Tfrm_LOSTA210Q.dte_fromExit(Sender: TObject);
begin
...
procedure Tfrm_LOSTA210Q.dte_toExit(Sender: TObject);
begin
     sts_Message.Panels[1].Text := '';
...
procedure Tfrm_LOSTA210Q.FormShow(Sender: TObject);
begin
  btn_resetClick(Sender);
end;

procedure Tfrm_LOSTA210Q.cmb_id_cdChange(Sender: TObject);
begin
  //ShowMessage('code = ' + cmb_id_cd_d[cmb_id_cd.ItemIndex].code);
  //ShowMessage('name = ' + cmb_id_cd_d[cmb_id_cd.ItemIndex].name);
...
procedure Tfrm_LOSTA210Q.grd_displayDrawCell(Sender: TObject; ACol,
...
procedure Tfrm_LOSTA210Q.grd_displayKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tfrm_LOSTA210Q.Copy1Click(Sender: TObject);
var
  select: TGridRect;
  str:String;
  i,j:Integer;
...
procedure Tfrm_LOSTA210Q.btn_queryClick(Sender: TObject);
var
	cmdStr:String;
    filePath:String;
    f:TextFile;
...
procedure Tfrm_LOSTA210Q.btn_excelClick(Sender: TObject);
begin
	Proc_gridtoexcel('분실폰전달내역조회(LOSTA210Q)',grd_display.RowCount, grd_display.ColCount, grd_display, 'LOSTA210Q');
end;
...
*******************************************************************************}
procedure Tfrm_LOSTA210Q.StringGrid_DrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; i_align : integer);
var
  LeftPos: Integer;
  TopPos : integer;
  CellStr: string;
...
procedure Tfrm_LOSTA210Q.dte_fromKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tfrm_LOSTA210Q.dte_toKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if Key = VK_RETURN then
      cmb_id_cd.SetFocus;
...
procedure Tfrm_LOSTA210Q.cmb_id_cdKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if Key = VK_RETURN then
...
procedure Tfrm_LOSTA210Q.btn_resetClick(Sender: TObject);
var i : Integer;
begin
  changeBtn(Self);
