...
begin
  Proc_gridtoexcel('우체국 출고건수 확인 (LOSTC250Q)',grd_display.RowCount, grd_display.ColCount, grd_display, 'LOSTC250Q');
end;
...
var
	head:TLOSTC250Q_PRT_HEAD;	//head 계산
	Canvas: TCanvas;
	i, j, page : integer;
  ...
  var
    ...
    len :Cardinal;
	begin
    centerPrint(prntWidth div 2,  	 '우체국 출고건수 확인');
    //상기 문자열에 대한 밑줄 긋기
    centerUnderLine(prntWidth div 2, '우체국 출고건수 확인');
...
begin
  ...
  //위치 중요
  head := TLOSTC250Q_PRT_HEAD.Create(prntMargin, prntWidth, Canvas);  //마진, 전체폭,...
