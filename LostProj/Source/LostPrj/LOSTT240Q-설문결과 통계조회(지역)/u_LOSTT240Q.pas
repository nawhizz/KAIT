...
var
  fm_LOSTT240Q: Tfm_LOSTT240Q;

implementation
uses u_LOSTT240Q_CHILD;

{$R *.dfm}

procedure Tfm_LOSTT240Q.FormCreate(Sender: TObject);
begin
...
procedure Tfm_LOSTT240Q.disableComponents;
begin
  
end;

procedure Tfm_LOSTT240Q.enableComponents;
begin
...
procedure Tfm_LOSTT240Q.InitComponent;
var
  i : Integer;
...
procedure Tfm_LOSTT240Q.btn_CloseClick(Sender: TObject);
begin
  Close;
end;

procedure Tfm_LOSTT240Q.FormShow(Sender: TObject);
var
  i:Integer;
...
begin
  ...
  //서비스 호출
	if not TMAX.Call('LOSTT110P') then goto LIQUIDATION;
...
{------------------------------------------------------------------------------}
procedure Tfm_LOSTT240Q.btn_InquiryClick(Sender: TObject);
var
...
begin
  ...
  TMAX.Connect;
    ...
    if (TMAX.SendString('INF002',common_username    ) < 0) then  goto LIQUIDATION;
    if (TMAX.SendString('INF002',common_usergroup   ) < 0) then  goto LIQUIDATION;
    if (TMAX.SendString('INF003','LOSTT240Q'        ) < 0) then  goto LIQUIDATION;
    ...
    //서비스 호출
    if not TMAX.Call('LOSTT240Q') then begin
    ...
    begin
      ...
      (*서울건수        *) Cells[ fInc(j),i+1] := convertWithCommer(  TMAX.RecvString ('INT103',i));
      (*부산건수        *) Cells[ fInc(j),i+1] := convertWithCommer(  TMAX.RecvString ('INT104',i));
      (*대구건수        *) Cells[ fInc(j),i+1] := convertWithCommer(  TMAX.RecvString ('INT105',i));
      (*인천건수        *) Cells[ fInc(j),i+1] := convertWithCommer(  TMAX.RecvString ('INT106',i));
      (*광주건수        *) Cells[ fInc(j),i+1] := convertWithCommer(  TMAX.RecvString ('INT107',i));
      (*대전건수        *) Cells[ fInc(j),i+1] := convertWithCommer(  TMAX.RecvString ('INT108',i));
      (*울산건수        *) Cells[ fInc(j),i+1] := convertWithCommer(  TMAX.RecvString ('INT109',i));
      (*경기건수        *) Cells[ fInc(j),i+1] := convertWithCommer(  TMAX.RecvString ('INT110',i));
      (*강원건수        *) Cells[ fInc(j),i+1] := convertWithCommer(  TMAX.RecvString ('INT111',i));
      (*충북건수        *) Cells[ fInc(j),i+1] := convertWithCommer(  TMAX.RecvString ('INT112',i));
      (*충남건수        *) Cells[ fInc(j),i+1] := convertWithCommer(  TMAX.RecvString ('INT113',i));
      (*전북건수        *) Cells[ fInc(j),i+1] := convertWithCommer(  TMAX.RecvString ('INT114',i));
      (*전남건수        *) Cells[ fInc(j),i+1] := convertWithCommer(  TMAX.RecvString ('INT115',i));
      (*경북건수        *) Cells[ fInc(j),i+1] := convertWithCommer(  TMAX.RecvString ('INT116',i));
      (*경남건수        *) Cells[ fInc(j),i+1] := convertWithCommer(  TMAX.RecvString ('INT117',i));
      (*제주건수        *) Cells[ fInc(j),i+1] := convertWithCommer(  TMAX.RecvString ('INT118',i));
      (*기타건수        *) Cells[ fInc(j),i+1] := convertWithCommer(  TMAX.RecvString ('INT119',i));
      (*합계건수        *) Cells[ fInc(j),i+1] := convertWithCommer(  TMAX.RecvString ('INT120',i));
      end;
    end;

    sts_Message.Panels[1].Text := ' ' + intToStr(TMAX.RecvInteger('INT100',0)) + '건이 조회 되었습니다.';
...
{------------------------------------------------------------------------------}
procedure Tfm_LOSTT240Q.btn_UpdateClick(Sender: TObject);
var
  i,j,cnt:Integer;
...
{------------------------------------------------------------------------------}
procedure Tfm_LOSTT240Q.btn_AddClick(Sender: TObject);
begin
  btn_UpdateClick(Sender);
end;

procedure Tfm_LOSTT240Q.btn_DeleteClick(Sender: TObject);
var
...
{------------------------------------------------------------------------------}
procedure Tfm_LOSTT240Q.cmb_gen_stChange(Sender: TObject);
var
  i,j : Integer;
begin
...
{------------------------------------------------------------------------------}
procedure Tfm_LOSTT240Q.cmb_topicChange(Sender: TObject);
var
  i,j:Integer;
...
{------------------------------------------------------------------------------}
procedure Tfm_LOSTT240Q.initStrGrid;
var
  j : Integer;
begin
  j := 0;
  with grd_display do begin
    ...
    ColWidths[fInc(j)] := 80;   // 서울
    ColWidths[fInc(j)] := 80;   // 부산
    ColWidths[fInc(j)] := 80;   // 대구
    ColWidths[fInc(j)] := 80;   // 인천
    ColWidths[fInc(j)] := 80;   // 광주
    ColWidths[fInc(j)] := 80;   // 대전
    ColWidths[fInc(j)] := 80;   // 울산
    ColWidths[fInc(j)] := 80;   // 경기
    ColWidths[fInc(j)] := 80;   // 강원
    ColWidths[fInc(j)] := 80;   // 충북
    ColWidths[fInc(j)] := 80;   // 충남
    ColWidths[fInc(j)] := 80;   // 전북
    ColWidths[fInc(j)] := 80;   // 전남
    ColWidths[fInc(j)] := 80;   // 경북
    ColWidths[fInc(j)] := 80;   // 경남
    ColWidths[fInc(j)] := 80;   // 제주
    ColWidths[fInc(j)] := 80;   // 기타
    ColWidths[fInc(j)] := 80;   // 합계
...
{------------------------------------------------------------------------------}
procedure Tfm_LOSTT240Q.grd_displayDrawCell(Sender: TObject; ACol,
  ARow: Integer; Rect: TRect; State: TGridDrawState);
var
  grid: TStringGrid;
  S:String;
...
{------------------------------------------------------------------------------}
procedure Tfm_LOSTT240Q.StringGrid_DrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; i_align : integer);
var
  LeftPos: Integer;
  TopPos : integer;
  CellStr: string;
...
procedure Tfm_LOSTT240Q.btn_ExcelClick(Sender: TObject);
begin
	Proc_gridtoexcel(TITLE + '(' + PGM_ID + ')',grd_display.RowCount, grd_display.ColCount, grd_display, PGM_ID);
end;

procedure Tfm_LOSTT240Q.btn_queryClick(Sender: TObject);
var
	cmdStr    :String;
  filePath  :String;
  f         :TextFile;
...
procedure Tfm_LOSTT240Q.btn_Search_TopicClick(Sender: TObject);
begin
  self.hide;
  fm_LOSTT240Q_CHILD.show;
end;

procedure Tfm_LOSTT240Q.btn_resetClick(Sender: TObject);
begin
  self.InitComponent;
end;
