...
var
  frm_LOSTD530L: Tfrm_LOSTD530L;

implementation
uses u_landprt;
{$R *.DFM}

procedure Tfrm_LOSTD530L.disableComponents;
begin
  disableBtn(Self);
...
procedure Tfrm_LOSTD530L.enableComponents;
begin
  changeBtn(Self);
...
procedure Tfrm_LOSTD530L.PrtFormShow;
begin
	// 출력 다이로그 박스
	frm_LANDPRT := Tfrm_LANDPRT.Create(self);
	frm_LANDPRT.FormSet('LostD530L.txt', lbl_Program_Name.Caption);
	try
...
procedure Tfrm_LOSTD530L.setEdtKeyPress;
var i : Integer;
begin
...
procedure Tfrm_LOSTD530L.Edt_onKeyPress ( Sender : TObject; var key : Char);
begin
 if (key = #13) then
...
{----------------------------------------------------------------------------}
procedure Tfrm_LOSTD530L.FormCreate(Sender: TObject);
begin
...
procedure Tfrm_LOSTD530L.btn_CloseClick(Sender: TObject);
begin
     close;
end;

procedure Tfrm_LOSTD530L.btn_PrintClick(Sender: TObject);
var
  count1,i : Integer;
...
begin
  ...
  TMAX.Connect;
	...
	if (TMAX.SendString('INF003','LOSTD530L') < 0) then  goto LIQUIDATION;
  ...
  //서비스 호출
  serviceSuccess := TMAX.Call('LOSTD530L');
...
procedure Tfrm_LOSTD530L.btn_ResetClick(Sender: TObject);
begin
...
procedure Tfrm_LOSTD530L.btn_queryClick(Sender: TObject);
var
	cmdStr:String;
    filePath:String;
    f:TextFile;
begin
    ...
    filePath:='..\Temp\LOSTD530L_QRY.txt';
...
procedure Tfrm_LOSTD530L.FormShow(Sender: TObject);
begin
