...
var
  frm_LOSTB580L: Tfrm_LOSTB580L;

implementation
uses  u_landprt;
{$R *.DFM}

procedure Tfrm_LOSTB580L.disableComponents;
begin
  disableBtn(Self);
...
procedure Tfrm_LOSTB580L.enableComponents;
begin
  changeBtn(Self);
...
procedure Tfrm_LOSTB580L.PrtFormShow;
begin
	// 출력 다이로그 박스
	frm_LANDPRT := Tfrm_LANDPRT.Create(self);
	frm_LANDPRT.FormSet('LostB580L.txt', lbl_Program_Name.Caption);
	try
...
procedure Tfrm_LOSTB580L.setEdtKeyPress;
var i : Integer;
begin
...
procedure Tfrm_LOSTB580L.Edt_onKeyPress ( Sender : TObject; var key : Char);
begin
 if (key = #13) then
...
{----------------------------------------------------------------------------}
procedure Tfrm_LOSTB580L.FormCreate(Sender: TObject);
begin
  {----------------------- 공통 어플리케이션 설정 ---------------------------}
   setEdtKeyPress;
...
procedure Tfrm_LOSTB580L.btn_CloseClick(Sender: TObject);
begin
     close;
end;

procedure Tfrm_LOSTB580L.dte_fromExit(Sender: TObject);
begin
...
procedure Tfrm_LOSTB580L.dte_toExit(Sender: TObject);
begin
	sts_Message.Panels[1].Text := '';
...
procedure Tfrm_LOSTB580L.btn_PrintClick(Sender: TObject);
var
  count1,i : Integer;
...
begin
  ...
  TMAX.Server := 'KAIT_LOSTPRJ';
	...
	if (TMAX.SendString('INF003','LOSTB580L') < 0) then  goto LIQUIDATION;
  ...
  //서비스 호출
  serviceSuccess := TMAX.Call('LOSTB580L');
...
procedure Tfrm_LOSTB580L.btn_ResetClick(Sender: TObject);
begin
...
procedure Tfrm_LOSTB580L.btn_queryClick(Sender: TObject);
var
	cmdStr:String;
    filePath:String;
    f:TextFile;
begin
    ...
    filePath:='..\Temp\LOSTB580L_QRY.txt';
...
procedure Tfrm_LOSTB580L.FormShow(Sender: TObject);
begin
