...
e_prevday( char src[], char dest[], char *mask )
#else
e_prevday( src, dest, mask )
char	src[];
char	dest[];
char	*mask;
#endif
{
	...
	if ( dateinfo.imonth == 1 && dateinfo.iday == 1 )
	{
		dateinfo.iyear--;
		dateinfo.imonth = 12;
		dateinfo.iday = 31;
	}
	else if ( dateinfo.iday == 1 )
	{
		dateinfo.imonth--;
		dateinfo.iday = dateinfo.lastday[dateinfo.imonth - 1];
	}
	else
		dateinfo.iday--;
