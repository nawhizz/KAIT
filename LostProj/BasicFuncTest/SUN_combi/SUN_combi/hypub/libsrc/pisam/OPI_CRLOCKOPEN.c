...
OPI_CRLOCKOPEN( char *retcode, int *fd, char *filepath, char *fileid )
#else
OPI_CRLOCKOPEN( retcode, fd, filepath, fileid )
...
#endif
{
	...
	if( ( *fd = PI_CRLOCKOPEN( l_filepath, l_fileid ) ) < 0 )
		pi_errset( retcode );
	else
		memset( retcode, ' ', 5 );
}
