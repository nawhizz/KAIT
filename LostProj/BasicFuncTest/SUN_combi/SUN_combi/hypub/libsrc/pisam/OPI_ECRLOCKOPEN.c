...
OPI_ECRLOCKOPEN( char *retcode, int *fd, char *fileid, char *fileext )
#else
OPI_ECRLOCKOPEN( retcode, fd, fileid, fileext )
...
#endif
{
	...
	if( ( *fd = PI_ECRLOCKOPEN( l_fileid, l_fileext ) ) < 0 ) 
		pi_errset( retcode );
	else
		memset( retcode, ' ', 5 );
}
