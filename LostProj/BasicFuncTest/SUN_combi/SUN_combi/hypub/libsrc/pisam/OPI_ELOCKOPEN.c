...
OPI_ELOCKOPEN( char *retcode, int *fd, char *fileid, char *fileext )
#else
OPI_ELOCKOPEN( retcode, fd, fileid, fileext )
...
#endif
{
	...
	if( ( *fd = PI_ELOCKOPEN( l_fileid, l_fileext ) ) < 0 ) 
		pi_errset( retcode );
	else
		memset( retcode, ' ', 5 );
}
