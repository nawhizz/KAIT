...
OPI_ETRBUILD( char *retcode, int *fd, char *fileid, char *fileext )
#else
OPI_ETRBUILD( retcode, fd, fileid, fileext )
...
#endif
{
	...
	if( ( *fd = PI_ETRBUILD( l_fileid, l_fileext ) ) < 0 ) 
		pi_errset( retcode );
	else
		memset( retcode, ' ', 5 );
}
