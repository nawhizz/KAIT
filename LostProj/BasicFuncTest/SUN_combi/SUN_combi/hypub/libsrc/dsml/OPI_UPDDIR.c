...
/*----------------------------------------------------------------------*
|									|
|	void OPI_UPDDIR( int *fd, char *olddirpath,			|
...
OPI_UPDDIR( int *fd, char *olddirpath, char *newdirpath,  char *retcode )
#else
OPI_UPDDIR( fd, olddirpath, newdirpath, retcode )
int			*fd;			/* S9(8) COMP */
...
#endif
{
	...
	if( PI_UPDDIR( l_fd, l_olddirpath, l_newdirpath ) < 0 )
		ds_errset( retcode );
	else
		memset( retcode, ' ', 5 );
}
