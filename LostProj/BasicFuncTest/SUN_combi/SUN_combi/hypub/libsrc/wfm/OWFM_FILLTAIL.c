/* OWFM_FILLTAIL () : LIB wfm */
/*----------------------------------------------------------------------*
|									|
|	void OWFM_FILLTAIL( char *retcode, int *ffd,  char *data	|
|			int *maxdatalen, char *mdata, int *mdatalen )	|
|									|
|	Fill Data into fields of tail segment. (merge)			|
|									|
...
OWFM_FILLTAIL( char *retcode, int *ffd, char *data, int *maxdatalen,
						 char *mdata, int *mdatalen )
#else
OWFM_FILLTAIL( retcode, ffd, data, maxdatalen, mdata, mdatalen )
char	*retcode;		/* X(5) */
int	*ffd;			/* S9(8) COMP */
char	*data;			/* X(len) */
...
#endif
{
	...
	if( ( l_mdata = WFmFillTail( l_ffd, l_data ) ) == (char *)0 )
	{
		wfm_errset( retcode );
		return;
	}
