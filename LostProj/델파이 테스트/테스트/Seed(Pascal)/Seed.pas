...
const
  ...
  SeedStrMaxLen = 200;                     // encrypt or decrypt string length
  SeedPbNHShopKey = 'K2I5I4S3L3C0H1U1';
...
const
  ...
  function seed_encrypt(var src: String; var src_size: Integer; var target: String; var target_size: Integer; var key: Array of BYTE):Integer;
...
function seed_encrypt(var src: String; var src_size: Integer; var target: String; var target_size: Integer):Integer;
var
   key : Array[0..15] of BYTE;
   round_key : Array[0..31] of LongWord;
   block : Array[0..SeedBlockSize] of BYTE;
   i, rt, result_size, block_size : Integer;

   temp_src : Array[0..SeedStrMaxLen] of BYTE;
   temp_target : Array[0..SeedStrMaxLen] of BYTE;
begin
   i := 0;
   rt := 0;
   result_size := 0;
   block_size := SeedBlockSize;

   FillChar(temp_src, SizeOf(temp_src), 0);
   FillChar(temp_target, SizeOf(temp_target), 0);

   Move( SeedPbNHShopKey, key, Length(SeedPbNHShopKey) );
   SeedEncRoundKey(round_key, key);

   temp_src := src;

   //src_size = add_padding(temp_src, src_size);

   rt := Trunc(src_size / SeedBlockSize);
   if (src_size Mod 16 <> 0) then rt := rt + 1;

   for i := 0 to rt -1 do
   begin
        //memcpy(block, temp_src + (i * SeedBlockSize), SeedBlockSize);
        //SeedEncrypt(block, round_key);
        //memcpy(temp_target + (i * SeedBlockSize), block, SeedBlockSize);
   end;

   Result := result_size;
end;

function nh_encrypt(var src: String; var target: String):Integer;
var
   tmp_target : String;
   len : Integer;
...
begin
   ...
   //len = seed_encrypt(src, strlen(src), tmp, sizeof(tmp), (BYTE*)SeedPbNHShopKey);
   //strncpy(target, tmp, strlen(tmp));

   target := '';
   len := seed_encrypt(src, length(src), tmp_target, SeedStrMaxLen);
   target := tmp_target;
